    ...
    [self.textStorage enumerateAttribute:AMBackgroundDrawableAttributeName
        ...
        if ([value conformsToProtocol:@protocol(AMDrawable)]) {
            ...
            if (isInline) {
                 ...
                 * 内联背景绘制（isInline = YES）
                 * 
                 * 适用场景：行内元素，如 `代码片段`、**粗体文本**、*斜体文本* 等
                 * 
                 * 绘制策略：逐行片段分别绘制
                 * - 当内容跨多行时，每一行都单独绘制一个背景矩形
                 * - 每行的背景边缘会根据位置进行裁剪（圆角处理）
                 * 
                 * 举例：假设有文本 "这是一段`很长的代码片段，可能会换行显示`的示例"
                 * 如果 `很长的代码片段，可能会换行显示` 跨越了两行：
                 * 第1行：[很长的代码片段，可能会] - 绘制背景，右边缘裁剪（不圆角）
                 * 第2行：[换行显示] - 绘制背景，左边缘裁剪（不圆角）
                 * 
                 * 这样每行都有独立的背景矩形，视觉上呈现为"分段式"背景
                 */
                 ...
                 * 块级背景绘制（isInline = NO）
                 * 
                 * 适用场景：块级元素，如代码块、引用块、表格等
                 * 
                 * 绘制策略：合并所有行片段为一个完整矩形
                 * - 先遍历所有相关的行片段，计算它们的联合矩形
                 * - 然后绘制一个完整的背景矩形，覆盖整个块级元素
                 * 
                 * 举例：假设有一个代码块：
                 * ```
                 * function hello() {
                 ...
                 * 块级绘制会：
                 * 1. 计算所有4行代码的外接矩形
                 * 2. 绘制一个完整的背景矩形，覆盖整个代码块区域
                 * 3. 背景是连续的，没有行间断裂，四个角都保持圆角
                 * 
                 * 视觉效果：整个代码块有一个统一的背景色，看起来像一个完整的区块
                 */
                __block CGRect bounds = CGRectNull;
                [self enumerateLineFragmentsForGlyphRange:range
                                               usingBlock:^(CGRect rect, CGRect usedRect, NSTextContainer * _Nonnull textContainer, NSRange glyphRange, BOOL * _Nonnull stop) {
                    // 计算每行的实际显示区域（减去行片段的内边距）
                    CGRect lineRect = UIEdgeInsetsInsetRect(rect, UIEdgeInsetsMake(0, textContainer.lineFragmentPadding, 0, textContainer.lineFragmentPadding));
                    
                    if (CGRectIsNull(bounds)) {
                        // 第一行：直接使用当前行的矩形作为初始边界
                        bounds = lineRect;
                    } else {
                        // 后续行：与之前的边界进行联合，扩展总边界
                        bounds = CGRectUnion(bounds, lineRect);
                    }
                }];
                
                // 使用合并后的完整矩形绘制背景，不进行边缘裁剪
                CGContextSaveGState(context);
    ...
    [self.textStorage enumerateAttributesInRange:charactorRange options:0 usingBlock:^(NSDictionary<NSAttributedStringKey, id> *attrs, NSRange range, BOOL *stop) {
        ...
        if ([attrs.allKeys containsObject:NSAttachmentAttributeName]) {
            ...
            if ([attrValue isKindOfClass:[AMIconLinkAttachment class]] || [attrValue isKindOfClass:[AMImageTextAttachment class]]) {
                ...
                rect.origin.x += origin.x;
                rect.origin.y += origin.y;
                // 在这里, 才将 attchView 的 frame 进行了真正的确定. 
